/* replace ecode with course code */

%let path=/home/&sysuserid/ESQ1M6;
...
%let url=%str(http://dl.liveweb.sas.com/eldata/esq1m6_oda.zip);
...
/* the file specified in the code below is the file that creates the data, libs, etc. */
%let setup=%superq(unzip)/data/setup.sas;    
...
%let path=%superq(unzip);

%include "%superq(setup)";

%mend unpack;

%unpack(%superq(path),esq1m6.zip,esq1m6)
